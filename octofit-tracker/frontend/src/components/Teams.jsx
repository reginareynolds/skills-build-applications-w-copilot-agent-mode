...
function Teams() {
  ...
  useEffect(() => {
    ...
    fetchResource(teamsEndpoint, controller.signal).then((payload) => setTeams(getItems(payload))).catch((requestError) => { if (requestError.name !== 'AbortError') setError(requestError.message) })
