...
function Activities() {
  ...
  useEffect(() => {
    ...
    fetchResource('activities', controller.signal).then((payload) => setActivities(getItems(payload))).catch((requestError) => { if (requestError.name !== 'AbortError') setError(requestError.message) })
