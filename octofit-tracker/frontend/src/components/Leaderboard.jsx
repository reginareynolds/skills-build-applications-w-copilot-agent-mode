...
function Leaderboard() {
  ...
  useEffect(() => {
    ...
    fetchResource('leaderboard', controller.signal).then((payload) => setEntries(getItems(payload))).catch((requestError) => { if (requestError.name !== 'AbortError') setError(requestError.message) })
