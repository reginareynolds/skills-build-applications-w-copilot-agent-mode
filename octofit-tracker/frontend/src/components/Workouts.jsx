...
function Workouts() {
  ...
  useEffect(() => {
    ...
    fetchResource(workoutsEndpoint, controller.signal).then((payload) => setWorkouts(getItems(payload))).catch((requestError) => { if (requestError.name !== 'AbortError') setError(requestError.message) })
